...
struct TrackRow: View {
    ...
    //
    // Navy inversion is now the sole "this is playing" indicator — no
    // more equalizer bars. For the non-prominent variant the position
    // number still anchors the left gutter (rendered in white); the
    // prominent variant mirrors its non-current layout (title flush at
    // x=24, no position column) so rows don't reshape on activation.
...
#Preview {
    ZStack {
        ...
        VStack(spacing: 0) {
            ...
            TrackRow(position: "3", title: "The National Anthem", artist: "Radiohead", duration: "5:51", isCurrent: true, onTap: {})
